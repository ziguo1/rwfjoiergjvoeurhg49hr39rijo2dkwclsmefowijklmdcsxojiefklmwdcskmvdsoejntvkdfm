...
Promise.all([
    mergeFiles(getParts("MinesweeperPlus.pck", 1, 3)),
    mergeFiles(getParts("MinesweeperPlus.wasm", 1, 3))
]).then(([pckUrl, wasmUrl]) => {
    window.fetch = async function (url, ...args) {
        if (url.endsWith("MinesweeperPlus.pck")) {
            return originalFetch(pckUrl, ...args);
        } else if (url.endsWith("MinesweeperPlus.wasm")) {
